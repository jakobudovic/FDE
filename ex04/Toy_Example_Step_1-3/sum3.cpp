...
static const char* findNextLine(const char* iter, const char* limit) {
   ...
   while (iter < limit8) {
      ...
      constexpr uint64_t pattern = broadcast('\n');
      constexpr uint64_t highBit = broadcast(0x80);
      constexpr uint64_t lowBits = ~highBit;

      auto originalHigh = block & highBit;
      auto asciiOnly = block & lowBits;

      auto misses = ((asciiOnly ^ pattern) + lowBits);
      auto allMisses = misses | originalHigh;

      auto hits = (~allMisses) & highBit;
      if (hits) {
         return iter + (__builtin_ctzll(hits)/8);
      } else {
         iter+=8;
      }
   }
